...
from .urls import BASE_URL, ROUTE_BASE, ROUTE_EVENT_HOME, ROUTE_NEW_EVENT, ROUTE_PREFIX_EVENTS, URL_EVENT_HOME, URL_EVENTS, URL_NEW_EVENT
...
async def create():
    ...
    await event_editor.render(
        on_back=partial(ui.open, ROUTE_BASE), on_save=partial(ui.open, URL_EVENT_HOME.format(event_id=event_editor.event_id))
    )
...
async def event_home(event_id: uuid.UUID, client: Client):
        ...
        with ui.column().classes("w-full"):
            ...
            for guest in guests_going + guests_not_going:
                with ui.row().classes("w-full justify-left items-center"):
                    ...
                    icons = {StatusEnum.GOING.value: "check", StatusEnum.NOT_GOING.value: "close", StatusEnum.MAYBE.value: "question_mark"}
                    icon_colors = {
                        StatusEnum.GOING.value: "color=green",
                        StatusEnum.NOT_GOING.value: "color=red",
                        StatusEnum.MAYBE.value: "color=yellow",
                    }
                    ui.icon(icons.get(guest["status"])).classes("text-2xl").props(icon_colors.get(guest["status"]))
    ...
    async def open_floating_editor(_id):
        # Initialize the event editor and render it in a floating dialog
        event_editor = EventEditor(_id)
        await event_editor.render(floating=True, on_save=event_editor.close, on_back=event_editor.close)
    ...
    async def render_event():
        ...
        with ui.column().classes("w-full justify-center pt-10"):
            ui.label(f"{event.get('name')}").classes("cursor-pointer text-5xl font-bold border-l-8 border-teal-500 pl-4").on(
                "click", lambda: open_floating_editor(event_id)
            )
            ui.label(f"Host: {event.get('organizer')}").classes("cursor-pointer text-xl border-l-8 border-teal-600 pl-4 pr-4").on(
                "click", lambda: open_floating_editor(event_id)
            )
            with ui.column().classes("cursor-pointer w-full justify-left border-l-8 border-amber-300 pl-4"):
                ui.label(f"{event_dates_to_str(event)}").classes("cursor-pointer text-xl").on("click", lambda: open_floating_editor(event_id))
                ui.label(f"{event_times_to_str(event)}").classes("cursor-pointer text-xl").on("click", lambda: open_floating_editor(event_id))
            ui.label(f"{event.get('location')}").classes("cursor-pointer text-xl border-l-8 border-orange-400 pl-4 pr-4").on(
                "click", lambda: open_floating_editor(event_id)
            )
            ui.label(f"{event.get('description')}").classes("cursor-pointer text-xl border-l-8 border-orange-600 pl-4 pr-4").on(
                "click", lambda: open_floating_editor(event_id)
            )
